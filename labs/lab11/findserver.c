...
int create_socket(struct timeval *tv) {
    ...
    // function's second argument, struct timeval *tv, in the function call to
    ...
    if (setsockopt(client_socket, SOCK_STREAM, SO_SNDTIMEO, (const char*)tv, sizeof(*tv)) != 0) {
        printf("Error: Cannot set socket options. %s.\n", strerror(errno));
        return -1;
    }
